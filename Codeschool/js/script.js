document.addEventListener('DOMContentLoaded',()=>{
    ...
    //Função responsável por subir progressivamente a sessão
    ...
    //Inicializar uma variavel contador 
    let counterValue = 0;

    //Logica: Divide o número alvo pela quantidade de frames (2000/20=1)
    const incrementAmount = targetNumber / (durationLimit / 20 );

    //Cria um lopping temporal a cada 20 milisegundo
    const updateVisualTimer = setInterval(() =>{
        ...
        if(counterValue >= targetNumber){
            ...
            clearInterval(updateVesualTimer);
        }else{
